...
int main()
{
    ...
    for(i=1; i<=t; i++)
    {
    	...
    	{
    		sort(a,a+n);
    		if(a[n-1]==b[n-1])cout << a[n-1] << " " << 0 << endl;
    		else cout << a[n-1] << " " << 1 << endl;
    	}
    	else if(!cnt)cout << sum << " " << 0 << endl;
    	else
    	{
    		for(j=0; j<cnt; j++)if(a[j]<0)ct++;
    		cout << sum << " " << cnt-ct << endl;
    	}
    }
