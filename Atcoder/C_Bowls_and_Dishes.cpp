...
int main()
{
    ll n,m,k,i,j,ans=0,cnt;
    cin >> n >> m;

    vpll dish(m);
    Forn(i,m)cin >> dish[i].ff >> dish[i].ss;

    cin >> k;
    vpll ball(k);
    Forn(i,k)cin >> ball[i].ff >> ball[i].ss;
    ...
    for(j=0; j<(1<<k); j++)
    {
        ...
        Forn(i,k)
        {
            if(j & (1<<i))select[ball[i].ff]++;
            else select[ball[i].ss]++;
            cout << j <<" & (1<<"<< i << ") : " << (j & (1<<i)) << endl;
        }
        cnt=0;
        Forn(i,m)
        {
            if(select[dish[i].ff] and select[dish[i].ss])
            {
                cnt++;
            }
        }
        for(i=1; i<=n; i++)cout << select[i] << " ";
        cout << endl;
        cout << cnt << endl;
        ans = max(ans,cnt);
    }
