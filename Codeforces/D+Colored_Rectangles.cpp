...
ll l,m,n,R[205],G[205],B[205],dp[205][205][205];
ll Fajlamo1(ll r,ll g,ll b)
{
	ll x=0,y=0,z=0;
 
	if(!(r|g) or !(r|b) or !(g|b))return 0;
	if(dp[r][g][b])return dp[r][g][b];
	if(r and g)x=R[r]*G[g]+Fajlamo1(r-1,g-1,b);
	if(r and b)y=R[r]*B[b]+Fajlamo1(r-1,g,b-1);
	if(g and b)z=G[g]*B[b]+Fajlamo1(r,g-1,b-1);
	return dp[r][g][b]=max(x,max(y,z)); 
}

/*ll Fajlamo(ll r,ll g,ll b)
{
	ll i,j,k;
    forn(i,r){
    	forn(j,g){
